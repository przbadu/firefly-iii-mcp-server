...
function formatCategory(c: FireflyCategory): string {
...
export function registerCategoryTools(server: McpServer): void {
  server.registerTool(
    ...
    async (params) => {
      try {
        const result = await apiRequest<{ data: FireflyCategory[]; meta: { pagination: { total: number; current_page: number; total_pages: number } } }>(
          "/categories", "GET", undefined, { limit: params.limit, page: params.page }
        );
        if (!result.data.length) return { content: [{ type: "text" as const, text: "No categories found." }] };
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        const result = await apiRequest<{ data: FireflyCategory }>("/categories", "POST", params);
        return { content: [{ type: "text" as const, text: `Category created: ${formatCategory(result.data)}` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: handleApiError(error) }], isError: true };
      }
    }
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        const result = await apiRequest<{ data: FireflyCategory }>(`/categories/${id}`, "PUT", updates);
        return { content: [{ type: "text" as const, text: `Category updated: ${formatCategory(result.data)}` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: handleApiError(error) }], isError: true };
      }
    }
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        return { content: [{ type: "text" as const, text: `Category ${params.id} deleted.` }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: handleApiError(error) }], isError: true };
      }
    }
