...
function formatTransaction(tx) {
    ...
    for (const split of attrs.transactions) {
        ...
        if (split.category_name)
            lines.push(`  Category: ${split.category_name}`);
        if (split.budget_name)
            lines.push(`  Budget: ${split.budget_name}`);
        if (split.tags?.length)
            lines.push(`  Tags: ${split.tags.join(", ")}`);
        if (split.notes)
            lines.push(`  Notes: ${split.notes}`);
    }
    return lines.join("\n");
}
// ── Tool Registration ────────────────────────────────────────────────────────
export function registerTransactionTools(server) {
    // ── Create Transaction ─────────────────────────────────────────────────────
    server.registerTool("firefly_create_transaction", {
...
Returns: The created transaction with its ID.`,
    ...
    // ── List Transactions ──────────────────────────────────────────────────────
    server.registerTool("firefly_list_transactions", {
...
Returns a paginated list of transactions with key details.`,
        ...
        try {
            ...
            const result = await apiRequest("/transactions", "GET", undefined, queryParams);
            const { data, meta } = result;
            const pag = meta.pagination;
            if (!data.length) {
                return {
                    content: [{ type: "text", text: "No transactions found matching your filters." }],
                };
            }
    ...
    // ── Get Transaction ────────────────────────────────────────────────────────
    server.registerTool("firefly_get_transaction", {
        ...
        try {
            const result = await apiRequest(`/transactions/${params.id}`);
            return {
                content: [{ type: "text", text: formatTransaction(result.data) }],
            };
        }
        catch (error) {
            return {
    ...
    // ── Update Transaction ─────────────────────────────────────────────────────
    server.registerTool("firefly_update_transaction", {
...
First use firefly_get_transaction to retrieve the current values, then pass only the changed fields here.`,
        ...
        try {
            ...
            if (updates.description !== undefined)
                txUpdate.description = updates.description;
            if (updates.date !== undefined)
                txUpdate.date = updates.date;
            if (updates.amount !== undefined)
                txUpdate.amount = updates.amount;
            if (updates.source_name !== undefined)
                txUpdate.source_name = updates.source_name;
            if (updates.destination_name !== undefined)
                txUpdate.destination_name = updates.destination_name;
            if (updates.category_name !== undefined)
                txUpdate.category_name = updates.category_name;
            if (updates.budget_name !== undefined)
                txUpdate.budget_name = updates.budget_name;
            if (updates.tags !== undefined)
                txUpdate.tags = updates.tags;
            if (updates.notes !== undefined)
                txUpdate.notes = updates.notes;
            const result = await apiRequest(`/transactions/${id}`, "PUT", { transactions: [txUpdate] });
            return {
                content: [
                    {
                        type: "text",
    ...
    // ── Delete Transaction ─────────────────────────────────────────────────────
    server.registerTool("firefly_delete_transaction", {
    ...
    // ── Search Transactions ────────────────────────────────────────────────────
    server.registerTool("firefly_search_transactions", {
...
Combine multiple filters with spaces.`,
        ...
        try {
            const result = await apiRequest("/search/transactions", "GET", undefined, {
                query: params.query,
                limit: params.limit,
                page: params.page,
            });
            ...
            if (!data.length) {
                return {
                    content: [
                        { type: "text", text: `No transactions found for query: "${params.query}"` },
                    ],
                };
            }
