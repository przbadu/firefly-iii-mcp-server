...
export function registerTagTools(server) {
    server.registerTool("firefly_list_tags", {
        ...
        try {
            const result = await apiRequest("/tags", "GET", undefined, { limit: params.limit, page: params.page });
    ...
    server.registerTool("firefly_create_tag", {
        ...
        try {
            const result = await apiRequest("/tags", "POST", params);
            return { content: [{ type: "text", text: `Tag created: **${result.data.attributes.tag}** (ID: ${result.data.id})` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error) }], isError: true };
        }
    });
    server.registerTool("firefly_update_tag", {
        ...
        try {
            ...
            const result = await apiRequest(`/tags/${encodeURIComponent(tag)}`, "PUT", { tag, ...updates });
            return { content: [{ type: "text", text: `Tag updated: **${result.data.attributes.tag}**` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error) }], isError: true };
        }
    });
    server.registerTool("firefly_delete_tag", {
        ...
        try {
            ...
            return { content: [{ type: "text", text: `Tag "${params.tag}" deleted.` }] };
        }
        catch (error) {
            return { content: [{ type: "text", text: handleApiError(error) }], isError: true };
        }
